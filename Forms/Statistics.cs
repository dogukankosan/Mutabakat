...
namespace ETARaporlama.Forms
{
    public partial class Statistics : DevExpress.XtraEditors.XtraForm
    {
        ...
        void GridList()
        {
            ...
            {
                myconnet.Open();
                SqlCommand command = new SqlCommand("select TOP 5  CARUNVAN ,CARBAKIYE from CARKART  order by CARBAKIYE desc", myconnet);
        ...
        private void eXCELALToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    chartControl1.ExportToPdf(fr.SelectedPath + "\\CariTop(5).pdf");
                    XtraMessageBox.Show(fr.SelectedPath+ "CariTop(5).pdf", "AKTARMA İŞLEMİ BAŞARILI", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception a)
            {
        ...
        private void rESİMALToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (fr.ShowDialog() == DialogResult.OK)
                {
                    chartControl1.ExportToImage(fr.SelectedPath + "\\CariTop(5).png",System.Drawing.Imaging.ImageFormat.Png);
                    XtraMessageBox.Show(fr.SelectedPath + "CariTop(5).png", "AKTARMA İŞLEMİ BAŞARILI", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception a)
            {
